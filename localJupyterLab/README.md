...
If we want to run a local instance of JupyterLab https://jupyter.org/, and have Python installed on our computer, we can create a virtual environment using the commands below. *Note: the virtual environment will consume ~190Mb of storage. You can remove the `.venv` folder once you are finished with it since you can always rebuild it later. A `.ipynb_checkpoints` folder will be automatically created if you run or edit a Notebook file within JupyterLab.*
...
```shell
# Bash commands
$ cd "/mnt/c/Users/pgale/LocalDev/dv-api-test/localJupyterLab"   # change this to your local copy of the `localJupyterLab` directory
